...
async function getAccessToken(): Promise<string> {
  ...
  if (!clientId || !secret || !baseUrl) {
    throw new Error("Missing required Interswitch environment variables");
  }
...
export async function initiateEscrowHold(params: EscrowHoldParams): Promise<EscrowHoldResponse> {
  const token = await getAccessToken();
  const baseUrl = process.env.INTERSWITCH_BASE_URL;
  if (!baseUrl) throw new Error("Missing INTERSWITCH_BASE_URL environment variable");
...
export async function generatePaycode(_amountKobo: number, _reference: string): Promise<string> {
